    ...
    this.user.subscribe((user: firebase.User) => {
      if (user != null) {
        ...
        this.currUser.valueChanges().subscribe(queriedItems => {
          ...
          Object.keys(queriedItems).map(prop => {
            if (i) {
              i = false;
...
export class ActivitiesComponent {
  ...
  onSubmit(formData) {
    ...
    data.push({ activity: formData.value.activity, date: firebase.database.ServerValue.TIMESTAMP }).then(success => {
